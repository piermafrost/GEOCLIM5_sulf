...
subroutine dynsoil_create_output(ID, output_path, run_name, xlevs, lon, lat, area, litho_frac, slope, &
  ...
  type(netcdf_output_var), dimension(nDYNSoutvar), intent(out):: outvar_info

  integer, parameter:: ndim=5
  character(len=30):: vartype(nDYNSoutvar)
  character(len=100):: dname(ndim), vname(nDYNSoutvar), units(nDYNSoutvar)
  character(len=500):: long_name(nDYNSoutvar)
  integer, dimension(nDYNSoutvar,ndim):: defdim
  logical, dimension(nDYNSoutvar):: writevar
  double precision, dimension(nDYNSoutvar):: fillval
  ...
  ! ---------------------
  namelist /DYN_OUTPUT_FILE/ file_name
  namelist /DYN_OUTPUT_DIM/ dname, units, vartype
  namelist /DYN_OUTPUT_VAR/ vname, units, defdim, writevar, long_name, fillval, vartype
  ...
  !<><><><><><><><><><><><><><><><>!
  read(unit=ID, nml=DYN_OUTPUT_FILE)
  !<><><><><><><><><><><><><><><><>!
  call check_namelist_def('Error: "file_name" not defined in "DYN_OUTPUT_FILE" namelist of main config file', char_var=file_name)
  ...
  ! Global attributes
  call put_att(  fid, NF90_GLOBAL, 'title',                     attribute_text='DynSoil module ouputs')
  ...
  !<><><><><><><><><><><><><><><><>!
  read(unit=ID, nml=DYN_OUTPUT_DIM)
  !<><><><><><><><><><><><><><><><>!
  ...
  !
  write(num, fmt="(I0)") i
  call check_namelist_def('Error: "dname('//trim(num)//')" not defined in "DYN_OUTPUT_DIM" namelist of main config file', &
                          char_var=dname(i))
  call check_namelist_def('Error: "units('//trim(num)//')" not defined in "DYN_OUTPUT_DIM" namelist of main config file', &
                          char_var=units(i))
  !
  ...
  ! dim #4: regolith vertical levels
  i = 4
  !
  write(num, fmt="(I0)") i
  call check_namelist_def('Error: "dname('//trim(num)//')" not defined in "DYN_OUTPUT_DIM" namelist of main config file', &
  ...
  call check_namelist_def('Error: "units('//trim(num)//')" not defined in "DYN_OUTPUT_DIM" namelist of main config file', &
                          char_var=units(i))
  !
  call def_dim(fid, dname(i), nDSlev, dimid(i))
  call def_var(fid, dname(i), vartype(i), dimid(i:i), (/.true./), dimvarid(i))
  ! dimension variable attributes:
  call put_att(fid, dimvarid(i), 'axis',     attribute_text='Z')
  call put_att(fid, dimvarid(i), 'positive', attribute_text='down')
  call put_att(fid, dimvarid(i), 'name',     attribute_text=dname(i))
  call put_att(fid, dimvarid(i), 'units',    attribute_text=units(i))
  ...
  ! read all information
  !<><><><><><><><><><><><><><><><>!
  read(unit=ID, nml=DYN_OUTPUT_VAR)
  !<><><><><><><><><><><><><><><><>!
  ...
  do i = 1,nDYNSoutvar
    ...
    if (writevar(i)) then
      ...
      write(num, fmt="(I0)") i
      call check_namelist_def('Error: "vname('//trim(num)//')" not defined in DYN_OUTPUT_VAR namelist of config/IO_CONDITIONS', &
                               char_var=vname(i))
      call check_namelist_def('Error: "units('//trim(num)//')" not defined in DYN_OUTPUT_VAR namelist of config/IO_CONDITIONS', &
                               char_var=units(i))
      call check_namelist_def('Error: "defdim('//trim(num)//',:)" not defined in DYN_OUTPUT_VAR namelist of config/IO_CONDITIONS', &
                               int_var=defdim(i,1))
  ...
  call put_var(fid, dimvarid(4), var_real1D=real(xlevs)     )
  ...
  if (outvar_info(3)%writevar) call put_var(fid, outvar_info(3)%id, var_real2D=real(reshape(slope, shape=(/nlon,nlat/))))
