...
# GEOCLIM's main configuration file
...
then                          #%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    # Write GEOCLIM IO info in local configuration storage
    # ----------------------------------------------------
    ...
    echo $GEOCLIM_IO_FILE > .config/geoclim_io_file
    echo $CONFIG_FILE > .config/config_file

    echo $GEOCLIM_IO_FILE_NLINES       >> .config/IOfile_structure
    echo $RUN_NAME_LINENUM             >> .config/IOfile_structure
    echo $COMBINE_INIT_LINENUM         >> .config/IOfile_structure
    echo $DYNSOIL_RESTART_MODE_LINENUM >> .config/IOfile_structure
    echo $DYNSOIL_INIT_LINENUM         >> .config/IOfile_structure

    echo $OUTPUTDIR >> .config/outputdir

    echo $COMBINE_RESTARTDIR >> .config/combine_restart
    echo $COMBINE_RESTART    >> .config/combine_restart

    echo $DYNSOIL_RESTARTDIR >> .config/dynsoil_restart
    echo $DYNSOIL_RESTART    >> .config/dynsoil_restart

    echo $RUN_NAME > .config/run_name

    echo 1 > .config/run_number


    for k in $STOP_TIMES
    do
        echo $k >> .config/stop_times
    done
    test -e .config/stop_times && tend=`head -n 1 .config/stop_times`

    for k in $COMBINE_DT
    do
        echo $k >> .config/combine_dt
    done
    test -e .config/combine_dt && dt=`head -n 1 .config/combine_dt`

    for k in $CONTWTH_NSKIP
    do
        echo $k >> .config/contwth_nskip
    done
    test -e .config/contwth_nskip && dt_contwth=`head -n 1 .config/contwth_nskip`

    for k in $DYNSOIL_NSKIP
    do
        echo $k >> .config/dynsoil_nskip
    done
    test -e .config/dynsoil_nskip && dt_dynsoil=`head -n 1 .config/dynsoil_nskip`

    for k in $COMBINE_PRINT_NSKIP
    do
        echo $k >> .config/combine_print_nskip
    done
    test -e .config/combine_print_nskip && tprint=`head -n 1 .config/combine_print_nskip`

    for k in $GEOGRAP_PRINT_NSKIP
    do
        echo $k >> .config/geograp_print_nskip
    done
    test -e .config/geograp_print_nskip && tprint_geog=`head -n 1 .config/geograp_print_nskip`

    for k in $DYNSOIL_PRINT_NSKIP
    do
        echo $k >> .config/dynsoil_print_nskip
    done
    test -e .config/dynsoil_print_nskip && tprint_dynsoil=`head -n 1 .config/dynsoil_print_nskip`
    ...
    rm -r -f .backup
    mkdir .backup
    cp $GEOCLIM_IO_FILE .backup/IO_CONDITIONS
    cp $CONFIG_FILE .backup/cond_p20.dat
